...
书页图片既要离线存在，也要能从阅读器的固定地址真实加载。生成和验收时必须同时满足：

- 把正文、封面和图注引用的资源复制到 `public/book/`，拒绝缺失、空文件和残留的远程图片地址；
- 图片由 `dangerouslySetInnerHTML` 等方式动态注入时，React 的 JSX `onError` 不会自动绑定到这些节点，必须在正文容器更新后为其中的 `img` 添加原生 `load` 与 `error` 监听；
- 初始化时检查已经完成但解码失败的缓存状态：`image.complete && image.naturalWidth === 0`；
- 第一次失败后等待很短时间，在原地址追加 `reader-retry=<时间戳>` 等缓存破坏参数重新请求一次；用 `data-*` 标记已重试状态，第二次失败后停止，不能形成无限请求；
- 图片重新加载成功后，重新计算页边批注卡片与连接线位置；章节切换或组件卸载时移除监听器并清理重试定时器；
- 本地启动器在打开浏览器前，除首页外还要等待 `/book/book-data.json` 可访问；书籍有图片时，再抽查至少一个被引用的图片地址。

含图书籍的浏览器验收不能只看文件存在。实际进入至少一个含图章节，读取正文图片的 `complete`、`naturalWidth` 和 `naturalHeight`，三者必须分别为 `true`、大于 0、大于 0；同时截图或目视确认图片没有破图占位符、没有被 CSS 隐藏，也没有挤乱页边批注。
