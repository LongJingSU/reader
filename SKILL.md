...
- 对书页正文中动态注入的每张图片绑定原生 `load` 与 `error` 监听；发现 `complete && naturalWidth === 0` 或收到加载错误时，追加一次缓存破坏参数重新请求，成功后重新计算页边批注位置，并用数据属性阻止无限重试。
...
验证脚本会检查书籍数据包、离线图片、图片自动恢复代码、必要阅读交互、本地绑定、启动脚本和正式构建。先修复所有失败项。只有本地书籍数据包和阅读器都真实存在时，才能报告完成。

书籍含有图片时，必须在后台浏览器中实际打开至少一个含图章节，并确认每张可见图片满足 `complete === true`、`naturalWidth > 0`、`naturalHeight > 0`。随后模拟一次图片加载错误或用等价方式验证重试请求，再确认图片恢复且批注布局没有错位。浏览器不可用时，至少逐一验证本地资源非空、HTTP 返回成功和响应类型正确，并明确说明尚未完成视觉验收。
